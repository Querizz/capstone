...
st.subheader('Olist')
st.write('Projek ini dilakukan menggunakan Open Data yang dipublikasikan langsung oleh pihak Olist melalui www.kagle.com')
...
if analisis == "Customer Analysis":
    ...
    top_city = df_customer_analysis[df_customer_analysis['order_purchase_timestamp'].dt.year == sales_line_tahun]['customer_city'].value_counts().head(10).to_frame().reset_index()
    ...
    top_product_data = df_order_items_analysis[df_order_items_analysis['order_purchase_timestamp'].dt.year == pilih_tahun]['product_category_name_english'].value_counts().head(10).to_frame().reset_index()
    ...
    st.header('Jumlah Produk Terjual Tiap Bulan')
